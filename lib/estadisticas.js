...
async function calcularEstadisticas() {
  ...
  const [[fechaRow], [totalesRow], porMes, topClientes, porUsuario] = await Promise.all([
    ...
    sql`
      SELECT
        ...
        COUNT(*) FILTER (WHERE drive_file_id IS NULL AND archivo_borrado_en IS NULL)::int AS sin_drive,
        COUNT(*) FILTER (WHERE archivo_borrado_en IS NOT NULL)::int AS sin_archivo
      FROM tomas
    ...
    sql`
      ...
      JOIN tomas t ON t.id = tc.toma_id
  ...
  return {
    ...
    // Tomas que siguen en Blob porque falló la copia a Drive. Deberían ser siempre 0.
    sinDrive: totalesRow?.sin_drive || 0,
    // Tomas cuyo archivo se borró para liberar espacio. El registro se conserva.
    sinArchivo: totalesRow?.sin_archivo || 0,
    porMes,
    topClientes,
    porUsuario,
  };
